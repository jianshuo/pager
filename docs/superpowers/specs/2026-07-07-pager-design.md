...
**03-daemon 计划补充决策（2026-07-07）**：
- conv→agentSessionId 映射本地持久化在 `~/.pager/state.json`（hub 不回传，重启后可 resume）。
- adapter 层用 fixture 回放测试：`record-fixture.mjs` 录一次真 SDK 消息流成 JSONL，测试断言翻译不变量（有 session、running→…→done、文本+最终 patch、tool_card）；SDK 升级解析失败时重录。
- `allow_always` v1 按 allow 处理（会话内白名单留 v2）；权限超时默认 3600s 自动 deny。
- v1 只装这台 Mac（launchd `dev.jianshuo.pager.daemon`）；VPS systemd 部署等 04-ios 后再做。
