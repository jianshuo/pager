...
- 测试栈：`@cloudflare/vitest-pool-workers` 实际用 0.12.21 + `isolatedStorage: false`（SQLite DO 的 WAL 文件与隔离存储不兼容）；wrangler.jsonc 需要 `nodejs_compat` flag 和 `observability.enabled`。
- 本计划代码里数处 `return this.x(...)` 在 try 内不 await（rejection 绕过 catch）已改为 `return await`。
- 终审加固（27aade4）：deliver 结果检查（false → status failed + 502）、send 循环 per-socket 隔离、daemon 重连接管旧 socket（1012）、非法 JSON 400、无 meta ingest 409、status 事件不再覆盖 lastMessage、**MachineDO 消息串行队列**（webSocketMessage 在 await 点交错会乱序落库，实测复现）。
- `POST /api/conversations` 返回 502 时会话+首条消息+failed 状态已存在——客户端应视为「已创建但失败」。
